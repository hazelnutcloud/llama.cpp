...
const Maker = struct {
    ...
    fn obj(m: *const Maker, name: []const u8, src: []const u8) *Compile {
        ...
        if (m.target.result.os.tag == .windows and m.llama_vulkan) {
            const vk_path = m.builder.graph.env_map.get("VK_SDK_PATH") orelse @panic("VK_SDK_PATH not set");
            o.addIncludePath(.{ .path = m.builder.pathJoin(&.{ vk_path, "include" }) });
        }
...
pub fn build(b: *std.Build) !void {
    ...
    make.llama_vulkan = llama_vulkan;
    ...
    // Executables
    const server = make.exe("server", "examples/server/server.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, sampling, console, grammar_parser, clip, llava, unicode });
    if (make.target.result.os.tag == .windows) {
        server.linkSystemLibrary("ws2_32");
    }

    const exes = [_]*Compile{
        make.exe("main", "examples/main/main.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, sampling, console, grammar_parser, clip, unicode }),
        make.exe("simple", "examples/simple/simple.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, sampling, console, grammar_parser, clip, unicode }),
        make.exe("parallel", "examples/parallel/parallel.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, sampling, console, grammar_parser, clip, unicode }),
        make.exe("quantize", "examples/quantize/quantize.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, unicode }),
        make.exe("perplexity", "examples/perplexity/perplexity.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, unicode }),
        make.exe("embedding", "examples/embedding/embedding.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, unicode }),
        make.exe("finetune", "examples/finetune/finetune.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, train, unicode }),
        make.exe("train-text-from-scratch", "examples/train-text-from-scratch/train-text-from-scratch.cpp", &.{ ggml, ggml_alloc, ggml_backend, ggml_quants, llama, common, buildinfo, train, unicode }),
        server,
    };
    ...
    for (exes) |e| {
        ...
        if (llama_vulkan) {
            if (make.target.result.os.tag == .windows) {
                ...
                e.addLibraryPath(.{ .path = b.pathJoin(&.{ vk_path, "Lib" }) });
                e.linkSystemLibrary("vulkan-1");
            } else {
